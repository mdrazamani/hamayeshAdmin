...
const UserEditModalForm: FC<Props> = ({user, isUserLoading}) => {
  ...
  const handleImageChange = async (event) => {
    ...
    try {
      const response = await profileImage(file, 'slider')

      if (response.data.status === 'success') {
        const imagePath = response.data.data.slider[0].path
  ...
  const [userForEdit] = useState<User>({
    ...
    order: user.order || initialUser.order,
      ...
      <form id='kt_modal_add_user_form' className='form' onSubmit={formik.handleSubmit} noValidate>
        ...
        >
          ...
          <div className='fv-row mb-7'>
            ...
            <label className='required fw-bold fs-6 mb-2'>
              {' '}
              {intl.formatMessage({id: 'AUTH.INPUT.ORDER'})}
            </label>
            ...
            {/* begin::Input */}
            <input
              placeholder={intl.formatMessage({id: 'AUTH.INPUT.ORDER'})}
